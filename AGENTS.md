...
10. Repository boundary: if a request points outside this repo or clearly belongs to another repo/project, pause before reading, editing, running commands, or generating artifacts there. Name the current repo, the requested external path/repo, and ask whether to proceed; do not silently change working directories across repos.
...
| Data Fetching | @tanstack/react-query 5 | Server state + caching (default `staleTime: Infinity`, `gcTime: 60m`, `retry: 1` set in `src/App.tsx`) |
...
| `scripts/generate-types.mjs` | Generates TS types from OpenAPI YAML specs in `api-specs/` |
| `scripts/setup-ai-harness.sh` | Generates symlinks from `.ai/` to `.cursor/`, `.claude/`, `.agents/`, `.codex/` |
...
| `.claude/`, `.agents/`, `.cursor/`, `.codex/` | Generated AI loader symlinks (gitignored) |
...
|---------------|-----------|
| New entity page | `.ai/skills/entity-table-architecture/SKILL.md`, `src/lib/entity-table/README.md`, `.ai/rules/open-ui-entity-page-pattern.mdc` |
| UI kit components | `src/components/ui-kit/index.ts`, `src/styles/design-tokens.css`, `.ai/rules/open-ui-uikit-components.mdc` |
| API integration | `.ai/skills/funnelflux-v2-api-openapi/SKILL.md`, `api-specs/`, `src/api/client.ts` |
