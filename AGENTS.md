...
> **This file is the canonical agent guide.** Tool-specific entrypoints (`CLAUDE.md`, `GEMINI.md`) are git-ignored symlinks to this file, and `.cursor/` / `.claude/` are generated from `.ai/`. Run `./scripts/sync-ai-config.sh` to materialize them locally. Edit this file and `.ai/`, never the generated copies. See [`.ai/README.md`](.ai/README.md).
