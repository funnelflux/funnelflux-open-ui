...
export function useCategoryStripTableFlow<T extends EntityGridRow>({
  ...
  const segments = useMemo(
    () => sortCategorySegments(
      buildCategorySegmentsFromRows(listFiltered, categoryMap, reportColumns),
      reportColumns,
      effectiveSorting,
    ),
    [listFiltered, categoryMap, reportColumns, effectiveSorting],
  )
  ...
  useEffect(() => {
    ...
    const pageIndex = pageIndexForEntityInAtomicCategorySegments(
      segments,
      revealEntityId,
      pagination.pageSize,
    )
