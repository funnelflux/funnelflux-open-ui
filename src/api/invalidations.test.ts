...
describe('invalidations', () => {
  ...
  it('invalidatePageDataAfterCsvImport refreshes pages and page categories', async () => {
    await invalidatePageDataAfterCsvImport(qc)
    expect(qc.invalidateQueries).toHaveBeenCalledWith({ queryKey: queryKeys.pages.all })
    expect(qc.invalidateQueries).toHaveBeenCalledWith({ queryKey: queryKeys.categories.all })
  })
