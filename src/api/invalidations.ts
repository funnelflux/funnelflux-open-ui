...
/**
 * After CSV import: pages grid plus category strip (import may create categories by name).
 */
export async function invalidatePageDataAfterCsvImport(qc: QueryClient): Promise<void> {
  await Promise.all([invalidatePageData(qc), invalidateCategoryData(qc, 'page')])
}
...
/* ─── Campaigns / funnels (grouping dropdowns, not campaign strip) ───── */

export async function invalidateCampaignFunnelAuxiliary(queryClient: QueryClient): Promise<void> {
  await Promise.all([
