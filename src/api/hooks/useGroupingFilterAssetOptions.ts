...
import { getDrilldownStaticFilterOptions } from '@/lib/drilldownStaticFilterOptions'
...
export function useGroupingFilterAssetOptions(groupBy: string, queryEnabled: boolean) {
  const staticOptions = useMemo(() => getDrilldownStaticFilterOptions(groupBy), [groupBy])
  const kind = useMemo(() => getGroupingFilterAssetKind(groupBy), [groupBy])
  const enabled = queryEnabled && kind !== null && staticOptions === null
