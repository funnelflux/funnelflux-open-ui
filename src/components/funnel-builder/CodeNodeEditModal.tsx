...
function CodeNodeEditForm({
  ...
  onClose,
  formId,
}: Omit<CodeNodeEditModalProps, 'open'> & { formId: string }) {
  ...
  return (
    <>
      ...
      <FormModalBody>
        ...
        : <form id={formId} onSubmit={handleFormSubmit} className="flex flex-col gap-4">
...
export function CodeNodeEditModal({ nodeId, codeType, open, onClose }: CodeNodeEditModalProps) {
  ...
  return (
    ...
    >
      ...
      {open ?
        <CodeNodeEditForm key={nodeId} nodeId={nodeId} codeType={codeType} onClose={onClose} formId={formId} />
      : null}
    </FormModal>
  )
}
