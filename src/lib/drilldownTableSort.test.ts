...
describe('drilldownSortParamFromReport', () => {
  ...
  it('maps node view metrics back to their report column name', () => {
    expect(drilldownSortParamFromReport(sort('nodeViews', true), [
      { name: 'Node Views', type: 'metric' },
    ] as ReportColumn[])).toEqual({
      sortingColumns: [{ columnName: 'Node Views', order: 'desc' }],
    })
  })
