...
import type { Report, ReportRow } from '@/types/stats'
...
function forEachEntityGridStatsQuery(
  queryClient: QueryClient,
  queryKeyPrefix: readonly unknown[],
  ...
  const queries = queryClient.getQueryCache().findAll({
    predicate: (q) => {
      const queryKey = q.queryKey as unknown[]
      if (!Array.isArray(queryKey) || queryKey.length < prefixLen + 2) return false
      for (let i = 0; i < prefixLen; i++) {
        if (queryKey[i] !== queryKeyPrefix[i]) return false
      }
      return queryKey[prefixLen] === ENTITY_GRID_STATS_KEY
    },
  })

  for (const query of queries) {
    const old = queryClient.getQueryData<Report>(query.queryKey)
    if (!old) continue
    queryClient.setQueryData(query.queryKey, updater(old))
  }
}

function removeRowsByEntityId(rows: ReportRow[], entityCellIndex: number, drop: Set<string>): ReportRow[] {
  return rows
    .filter((row) => !drop.has(String(row.cells?.[entityCellIndex]?.raw ?? '')))
    .map((row) => {
      if (!row.children?.length) return row
      return { ...row, children: removeRowsByEntityId(row.children, entityCellIndex, drop) }
    })
}

function removeIdsFromStatsCaches(
  queryClient: QueryClient,
  queryKeyPrefix: readonly unknown[],
  ids: Iterable<string>,
): void {
  ...
  forEachEntityGridStatsQuery(queryClient, queryKeyPrefix, (report) => {
    const entityCellIndex = entityCellIndexForReportColumns(report.columns ?? [])
    const rows = removeRowsByEntityId(report.rows ?? [], entityCellIndex, drop)
    const removed = Math.max(0, (report.rows?.length ?? 0) - rows.length)
    if (removed === 0) return report

    return {
      ...report,
      rows,
      rowsReturned: Math.max(0, (report.rowsReturned ?? rows.length) - removed),
      rowsTotal: Math.max(0, (report.rowsTotal ?? rows.length) - removed),
      paging: report.paging
        ? {
            ...report.paging,
            totalRecords: Math.max(0, (report.paging.totalRecords ?? rows.length) - removed),
          }
        : report.paging,
    }
  })
...
export function removePageFromEntityGridCaches(queryClient: QueryClient, idPage: string): void {
  ...
  removeIdsFromStatsCaches(queryClient, queryKeys.pages.all, [idPage])
}
