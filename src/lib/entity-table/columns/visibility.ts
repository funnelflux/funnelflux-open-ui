...
import { buildChooserGroupsForPage, flatColumnsFromGroups, getColumnMeta, type MetricScope } from '@/components/ui-kit/data-table/columnRegistry'
...
export function useEntityGridColumnVisibility(
  ...
  const controlledColumnIds = useMemo(() => {
    const ids = new Set(flatColumnsFromGroups(buildChooserGroupsForPage(options?.hideScopes)).map((column) => column.id))
