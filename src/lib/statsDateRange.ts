...
/** Default entity-table window: last 365 calendar days through end of today in `timezone`. */
export function createDefaultEntityTableDateRange(timezone = 'UTC'): DateRange {
...
/** Default dashboard window: last 30 calendar days through end of today in `timezone`. */
export function createDefaultDashboardDateRange(timezone?: string): DateRange {
  const resolvedTimezone = timezone ?? (
    typeof Intl !== 'undefined'
      ? Intl.DateTimeFormat().resolvedOptions().timeZone
      : 'UTC'
  )
  return getPresetRange('last30', resolvedTimezone)
}

/**
 * Stable React Query key segment for reporting-timezone drilldown requests.
 */
export function dateRangeQueryKeyForReporting(
  from: Date,
  to: Date,
  timezone: string,
): readonly [string, ApiDateTimeRange] {
  return [timezone, toApiDateTimeRangeForReporting(from, to, timezone)] as const
}

/**
 * Stable React Query key segment for stats/list requests.
 * Uses the same {@link toApiDateTimeRange} shape as drilldown bodies, not raw ISO timestamps.
 */
export function dateRangeQueryKey(
  from: Date,
  to: Date,
  timezone: string,
): readonly [string, ApiDateTimeRange] {
  return [timezone, toApiDateTimeRange(from, to)] as const
}
