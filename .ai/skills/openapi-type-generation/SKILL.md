...
description: >-
  Regenerate TypeScript types from committed OpenAPI YAML in api-specs/
  after contract changes. Use when editing admin/api/v2/*/definition.yaml (then
  sync into api-specs), src/types/, or when the user mentions generate
...
- Editing OpenAPI specs: canonical PHP sources `admin/api/v2/*/definition.yaml`; committed inputs for the UI package are `api-specs/*-api.yaml` (keep them in sync when the backend contract changes)
...
1. **Sync** `admin/api/v2/{data,stats,ui,system}/definition.yaml` into `api-specs/{data,stats,ui,system}-api.yaml` (or edit the docs copies directly when the backend PR lands together).
2. **Run** the generator:
...
1. Add the definition to the appropriate file under `api-specs/` (and the PHP `definition.yaml` if applicable).
...
- See `.ai/rules/open-ui-type-generation.mdc` for full mapping rules.

### Architecture

```
api-specs/data-api.yaml     ─┐
api-specs/stats-api.yaml    ─┤
api-specs/ui-api.yaml       ─┼──▶ scripts/generate-types.mjs
api-specs/system-api.yaml   ─┘         │
                                                ▼
                                   src/types/generated/
                                   ├── data.ts
                                   ├── stats.ts
